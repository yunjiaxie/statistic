Template.personType.helpers({
	'item1': function  () {
		var data=Employees.find({personType:"本校在编人员"}).count();
		Session.set('item1',data);
		//console.log(data);
		return	data;
	},
	'item2': function () {
	 	var data=Employees.find({personType:"外单位借用人员"}).count();
	 	//console.log(data);
	 	Session.set('item2',data);
	 	return data;
	},
	'item3': function () {
	 	var data=Employees.find({personType:"编外人员"}).count();
	 	//console.log(data);
